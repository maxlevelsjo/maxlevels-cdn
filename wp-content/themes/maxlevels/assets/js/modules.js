...
(function ($) {
	...
	var mkdfImport = {
		...
		importAttachments: function (repeat) {
			...
			if (!repeat) {
				for (var i = 1; i < 20; i++) {
					var xml = i < 20 ? 'foton_content_' + i + '.xml' : 'foton_content_' + i + '.xml';
					var data = {
						action: 'content',
						xml: xml,
						images: mkdfImport.importImages
...
(function ($) {
    ...
    */
    ...
    function horizontalLayerSliderInit(slider, mouseWheelControl) {
        var swiperSlider = new Swiper(slider[0], {
            loop: true,
            parallax: true,
            speed: 1000,
    ...
    function mkdfInitVerticalShowcase() {
        ...
        if (verticalShowcase.length) {
            verticalShowcase.each(function () {
                ...
                var swiperSlider = new Swiper (swiperInstance[0], {
                    loop: false,
                    direction: 'vertical',
                    slidesPerView: 1,
                    mousewheel: {
				...
				afterRender: function () {
					...
					if (contactForm7.length) {
						contactForm7.each(function(){
							...
							thisForm.find('.wpcf7-submit').off().on('tap click', function(e){
								e.preventDefault();

								wpcf7.submit( thisForm[0] );
							});
						});
					}
					...
					//prepare contact form 7 clones - only working if using one form in split slider
					if (mkdf.windowWidth <= 1024) {

						var contactForm7Clones = $('.mkdf-vss-responsive div.wpcf7 > form');
						if (contactForm7Clones.length) {
							contactForm7Clones.each(function(){
								var thisForm = $(this);

								thisForm.replaceWith( contactForm7 );
							});
						}
					}
